...
function init() {
	$ui.register((ctx) => {
		...
		async function updateTraktSeasonProgress(traktId: number, seasonNumber: number, seasonId: number, progress: number) {
			...
			// Build map of existing watched episodes
			const existingWatchedMap = new Map<number, string>();
			for (const item of watchedHistory) {
				const ep = item.episode;
				if (ep?.number != null && item.watched_at) {
					existingWatchedMap.set(ep.number, item.watched_at);
				}
			}

			// Determine which episodes need to be added
			const episodesToAdd = [];
			const now = new Date().toISOString();

			for (let epNumber = 1; epNumber <= progress; epNumber++) {
				if (!existingWatchedMap.has(epNumber)) {
					// Only add episodes that aren't already in history
					episodesToAdd.push({
						ids: {
							trakt: traktId,
							season: seasonNumber,
			...
			// If there are episodes to add, send them
			if (episodesToAdd.length > 0) {
				const res = await traktTokenManager.traktFetch("/sync/history", {
					method: "POST", // POST is correct for adding new items
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({ episodes: episodesToAdd }),
				});
			...
			return { added: 0, existing: episodesToAdd.length };
		}
